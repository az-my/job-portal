...
**Job Aggregator** — a local-first job listing aggregator that scrapes Indonesian job boards (JobStreet, Dealls, Kalibrr, and Glints), normalizes the results into a single schema, and displays them in a high-density, searchable data table. Built with Next.js 16 (App Router), React 19, Tailwind CSS v4, and shadcn/ui.
...
1. **Collect** — four source adapters fetch raw listings:
   ...
   - **Glints** (`scraper/glints.py`): POSTs the `searchJobsV3` GraphQL query to `glints.com/api/v2-alc/graphql` with `sortBy=LATEST` (updatedAt desc). Anonymous access allows only page 1 with max 50 jobs (page 2+ returns 403 login-required); the collector stops gracefully at the wall.
...
| `src/app/api/query/` | Jobs-page natural-language query API: Gemini translates questions to read-only SQL (needs `GEMINI_API_KEY` in `.env.local`) |
...
| `scraper/jobstreet.py`, `scraper/dealls.py`, `scraper/kalibrr.py`, `scraper/glints.py` | Source collectors |
...
- **`Job`** — `id`, `title`, `company`, `location`, `type` (`full-time` | `part-time` | `remote` | `contract`), `description`, `salary`, `source` (`jobstreet` | `dealls` | `kalibrr` | `glints`), `sourceId` (dedup key, scoped per source), `url`, `logoUrl`, `requirements`, `raw` (stringified original API response), `createdAt` (real posting date from each source).
...
python scraper/main.py 5 glints       # Glints (one page of 50 max, anonymous cap)
```
...
- All four sources are scraped sequentially; a slow source delays the others (fine at 3 sources).
- JobStreet's GraphQL schema is unofficial and could change without notice; same for the Dealls, Kalibrr, and Glints endpoints. Glints anonymously serves only the newest ~50 jobs per run.
